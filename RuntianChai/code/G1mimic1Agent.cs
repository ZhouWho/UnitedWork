...
public class G1mimic1Agent : Agent
{
    ...
    */
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // --- 2. 新增：未来帧状态观测 (Look-ahead) ---
        int lookAheadFrames = 10; // 往前看 10 帧 (由于 deltaTime 是 0.01，这相当于看 0.1 秒后)
        int futureIndex = Mathf.Min(currentFrame + lookAheadFrames, itpData.Count - 1);
        
        if (itpData.Count > 0)
        {
            float[] futureData = itpData[futureIndex];
            float[] futurePos = new float[3];
            float[] futureRot = new float[4];
            float[] futureDof = new float[29];
            
            Array.Copy(futureData, 0, futurePos, 0, 3);
            Array.Copy(futureData, 3, futureRot, 0, 4);
            Array.Copy(futureData, 7, futureDof, 0, 29);

            // 观测未来的相对位置偏移 (3维)
            Vector3 targetFuturePos = new Vector3(-futurePos[1], futurePos[2], futurePos[0]);
            sensor.AddObservation(targetFuturePos - body.position);

            // 观测未来的关节目标角度 (29维)
            for (int i = 0; i < 29; i++)
            {
                sensor.AddObservation(futureDof[i]); 
            }
        }
        else
        {
    ...
    */
    ...
    void FixedUpdate()
    {
        ...
        if(tt > 1)
        {
            ...
            // 将角度差归一化
            float rotError = Quaternion.Angle(bodyRotNoY, newRotNoY) / 180f; 
            
            // 2. 计算根节点位置误差的平方
            float posErrorSqr = (body.position - newPosition).sqrMagnitude;
            
            // 3. 计算所有关节角度 (DoF) 的误差平方和
            float dofErrorSqrSum = 0f;
            for (int i = 0; i < 29; i++) 
            {
                float diff = jh[i].jointPosition[0] - currentDof[i];
                dofErrorSqrSum += (diff * diff); 
            }
            
            // --- 核心修改：指数奖励 (Exponential Reward) ---
            // 系数控制了要求有多“严苛”。误差为0时得分为1，误差越大得分平滑衰减到0。
            float rot_reward = Mathf.Exp(-5f * rotError * rotError);
            float pos_reward = Mathf.Exp(-10f * posErrorSqr);
            float dof_reward = Mathf.Exp(-2f * dofErrorSqrSum); 
            
            // 总奖励：姿态占大头，位置和旋转辅助
            float reward = (0.5f * dof_reward) + (0.3f * pos_reward) + (0.2f * rot_reward);
            AddReward(reward);
